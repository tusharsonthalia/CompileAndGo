...
func normalizeOutput(s string) string {
	// Standardize all whitespace by splitting and joining
	fields := strings.Fields(s)
	return strings.Join(fields, "\n")
}

func TestLLVMStack(t *testing.T) {
	root := "../benchmarks"
	...
	for _, d := range entries {
		...
		t.Run(benchName, func(t *testing.T) {
			...
			cmd := exec.Command("go", "run", golitePath, "-target=arm64-apple-macosx14.0.0", "-llvm-stack", tmpGoliteFile)
